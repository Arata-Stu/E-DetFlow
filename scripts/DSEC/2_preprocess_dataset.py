...
# デフォルト解像度 (DSEC)
dataset_2_height = {'DSEC': 480}
dataset_2_width = {'DSEC': 640}
...
class H5Reader:
    def __init__(self, h5_file: Path, dataset: str = 'DSEC'):
        ...
        # t_offsetの取得 (DSEC特有)
        self.t_offset = self.h5f['t_offset'][()] if 't_offset' in self.h5f else 0
    ...
    def time(self) -> np.ndarray:
        ...
        if self.all_times is None:
            raw_t = np.asarray(self.h5f['events']['t'], dtype='int64')
            self.all_times = raw_t + self.t_offset
...
def apply_filters(labels: np.ndarray, split_type: SplitType, filter_cfg: DictConfig, dataset_type: str = 'DSEC') -> np.ndarray:
...
def get_base_delta_ts_for_labels_us(unique_label_ts_us: np.ndarray, dataset_type: str = 'DSEC') -> int:
...
def labels_and_ev_repr_timestamps(npy_file: Path, split_type: SplitType, filter_cfg: DictConfig, align_t_ms: int, ts_step_ev_repr_ms: int, dataset_type: str):
    ...
    # DSEC対応: align_t_us は「シーケンス開始からのオフセット」として扱う
    # ラベルの最小時間を基準とする
    start_t_us = unique_ts_us[0]
    align_t_us = align_t_ms * 1000
    delta_t_us = ts_step_ev_repr_ms * 1000

    # 最初のフレームを決定 (基準時刻 + align分以降で最も近いラベル)
    unique_ts_idx_first = np.searchsorted(unique_ts_us, start_t_us + align_t_us, side='left')
    if unique_ts_idx_first >= len(unique_ts_us):
        unique_ts_idx_first = len(unique_ts_us) - 1
    ...
    ev_repr_timestamps_us_end = list(reversed(range(frame_timestamps_us[0], frame_timestamps_us[0] - align_t_us, -delta_t_us)))[::-1]
    
    # フレーム間のタイムスタンプを線形補間（またはステップ）で生成
    ...
    ev_repr_timestamps_us_end = np.unique(np.asarray(ev_repr_timestamps_us_end, dtype='int64'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', default='DSEC', help='DSEC') 
    ...
    for split_name in ['train', 'val', 'test']:
        ...
        for sequence_id in sequence_ids:
            ...
            out_ev_repr_path.mkdir(parents=True, exist_ok=True)
