...
def build_sincos2d_pos_embed(
    ...
    grid = torch.stack(
        torch.meshgrid([torch.arange(s, device=device, dtype=dtype) for s in feat_shape], indexing='ij')).flatten(1).transpose(0, 1)
...
def build_fourier_pos_embed(
    ...
    if in_pixels:
        grid = torch.stack(torch.meshgrid(
            [torch.linspace(-1., 1., steps=s, device=device, dtype=dtype) for s in feat_shape], indexing='ij')).flatten(1).transpose(0, 1)
    else:
        grid = torch.stack(torch.meshgrid(
            [torch.arange(s, device=device, dtype=dtype) for s in feat_shape], indexing='ij')).flatten(1).transpose(0, 1)
