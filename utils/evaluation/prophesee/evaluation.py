...
def evaluate_list(result_boxes_list,
    ...
    assert camera in {'gen1', 'gen4', 'VGA', 'SEVD'}, f'{camera=}'
    ...
    if camera == 'gen1':
        ...
        classes = ("car", "pedestrian", "two-wheeler", "misc")
    else:
    ...
    if apply_bbox_filters:
        ...
        min_box_diag = 60 if camera in {'gen4', 'VGA', 'SEVD'} else 30
        # In the supplementary mat, they say that min_box_side is 20 for gen4.
        min_box_side = 20 if camera in {'gen4', 'VGA', 'SEVD'} else 10
        if downsampled_by_2:
